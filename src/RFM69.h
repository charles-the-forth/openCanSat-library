...
#define RF69_MAX_DATA_LEN       55//61 // to take advantage of the built in AES/CRC we want to limit the frame size to the internal FIFO size (66 bytes - 3 bytes overhead - 2 bytes crc)
...
#define RF69_CSMA_LIMIT_MS 2000
#define RF69_TX_LIMIT_MS   2000
...
class RFM69 {
  public:
    ...
    static volatile uint32_t SENDERID;
    static volatile uint32_t TARGETID; // should match _address
    ...
    bool canSend();
    virtual void send(uint32_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK=false);
    virtual bool sendWithRetry(uint32_t toAddress, const void* buffer, uint8_t bufferSize, uint8_t retries=2, uint8_t retryWaitTime=40); // 40ms roundtrip req for 61byte packets
    virtual bool receiveDone();
    bool ACKReceived(uint32_t fromNodeID);
    bool ACKRequested();
    ...
    void sleep();
    int8_t readTemperature(int8_t calFactor=0); // get CMOS temperature (8bit)
  ...
  protected:
    ...
    virtual void sendFrame(uint32_t toAddress, const void* buffer, uint8_t size, bool requestACK=false, bool sendACK=false);
