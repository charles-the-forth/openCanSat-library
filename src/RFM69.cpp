...
volatile uint32_t RFM69::SENDERID;
volatile uint32_t RFM69::TARGETID;     // should match _address
...
uint32_t RFM69::transfer(unsigned long value)
{
  FourByte data = {value};
  for(byte i = 0; i < 4; i++)
  {
...
bool RFM69::initialize(uint8_t freqBand, uint32_t nodeID, uint8_t networkID)
{
...
void RFM69::send(uint32_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK)
{
  writeReg(REG_PACKETCONFIG2, (readReg(REG_PACKETCONFIG2) & 0xFB) | RF_PACKET2_RXRESTART); // avoid RX deadlocks
  uint32_t now = millis();
  while (!canSend() && millis() - now < RF69_CSMA_LIMIT_MS) receiveDone();
  if(!canSend() && millis()-now > RF69_CSMA_LIMIT_MS){
    //Serial.println(" RF69_CSMA_LIMIT_MS expired "); //Serial.print(millis() - sentTime);
    return;
  }else{
...
bool RFM69::sendWithRetry(uint32_t toAddress, const void* buffer, uint8_t bufferSize, uint8_t retries, uint8_t retryWaitTime) {
  uint32_t sentTime;
  for (uint8_t i = 0; i <= retries; i++)
  {
    //Serial.println("Right before send MESSAGE");
...
bool RFM69::ACKReceived(uint32_t fromNodeID) {
  if (receiveDone())
...
// internal function
void RFM69::sendFrame(uint32_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK)
{
...
int8_t RFM69::readTemperature(int8_t calFactor) // returns centigrade
{
  setMode(RF69_MODE_STANDBY);
