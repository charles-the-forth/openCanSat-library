...
{
    return String(getYear()) + " " + String(getMonth()) + " " + String(getDay()) + " " + String(getHour()) + " " + String(getMinute()) + " " + String(getSecond());
}
...
double OpenCansatGPS::convertToDecimalDeg(const String &line)
{

    int positionOfDot = line.indexOf(".");

	double deg = line.substring(0, positionOfDot - 2).toDouble();
	double min = line.substring(positionOfDot - 2).toDouble();
...
bool OpenCansatGPS::parseGNGLL(const String &line)
{
    ...
    if (time.length() == 9)
    {
        ...
        log(String("[scan] datetime = ") + String(timeDate.hrs) + " " + String(timeDate.min) + " " + String(timeDate.sec));
    }
